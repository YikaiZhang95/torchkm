...
TorchKM, cuML and Falkon with every method on the full
kernel (no Nyström centres anywhere): the same RBF bandwidth, the same
50-value λ grid from 1e-2 down to 2e-5, identical stratified 10-fold splits, float64 throughout,
...
- **cuML `SVC`** is the direct competitor: the same hinge objective, solved
  by SMO, tuned by the same fold loop (501 fits per repeat).
...
- **KeOps** is a kernel-operation engine rather than a model-selection
  library, so it goes to related work; the script can still run kernel ridge
  regression on a `LazyTensor` kernel as an optional extra column.
- **EigenPro 2** is cited; an optional runner (epochs tuned on the same
  folds) is in the script if the reviewers want a third GPU solver in the table.
...
- Preparing this benchmark exposed a defect in TorchKM's KQR
  ...
  exact fold solutions, as is the intercept line search of the DWD, logistic
  and squared-hinge solvers (see the changelog).
