...
def _eigh_on_host(K: torch.Tensor) -> EighResult:
...
def _eigh_once(K: torch.Tensor, backend: str) -> EighResult:
    """One factorisation of a CUDA tensor with an explicit backend, no retry."""
    if backend == "cusolver":
        return _eigh_with_library(K, "cusolver")
    if backend == "magma":
        return _eigh_with_library(K, "magma")
    if backend == "cpu":
        return _eigh_on_host(K)
...
def _run_attempts(
    ...
    and any failure of the last backend, propagates.
    """
    failed: List[str] = []
    for i, name in enumerate(order):
        last = i == len(order) - 1
        try:
            return _eigh_once(K, name), name, failed
...
def kernel_eigh(
    K: torch.Tensor, backend: str = "auto", *, return_info: bool = False
    ...
    if device.type != "cuda":
        w, U = torch.linalg.eigh(K)
        used = "lapack"
    else:
        (w, U), used, failed = _run_attempts(K, _attempt_order(requested))
    _synchronize(device)
